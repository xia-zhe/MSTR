...
class AVCA(nn.Module):
    def __init__(self, params_model, input_size_audio, input_size_video):
        ...
        self.trl_v = TRL(ranks=(400, 1, 1, 300), input_size=(64, 512, 1, 1), output_size=(64, 300))
        self.trl_a = TRL(ranks=(400, 1, 1, 300), input_size=(64, 512, 1, 1), output_size=(64, 300))
    ...
    def forward(self, audio, image, negative_audio, negative_image, word_embedding, negative_word_embedding):
        ...
        self.phi_at = self.trl_a(audio.reshape((64, 512, 1, 1)))
        self.phi_vt = self.trl_v(image.reshape((64, 512, 1, 1)))

        self.phi_at_neg = self.trl_a(negative_audio.reshape((64, 512, 1, 1)))
        self.phi_vt_neg = self.trl_v(negative_image.reshape((64, 512, 1, 1)))
